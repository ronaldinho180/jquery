...
<form id="actor-selection" action="index.php" method="post">
	...
	<button type="select">Go!</button>
</form>


<ul class="actors_list">
	<?php foreach ($actors as $a) {
			echo "<li data-actor_id='{$a->actor_id}'><a href='actors.php?actor_id={$a->actor_id}'>{$a->first_name} {$a->last_name}</a></li>";
		}
	?>
	...
	<script id="actor_list_template" type="text/x-handlebars-template">
		{{#each this}}
			<li data-actor_id="{{actor_id}}">
				<a href="actors.php?actor_id={{actor_id}}">{{fullName this}}</a>
			</li>
		{{/each}}
	</script>
